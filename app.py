...
def format_datetime(value, format='medium'):
  ...
  return babel.dates.format_datetime(date, format) #type: ignore
...
def delete_venue():
  ...
  try:
    ...
    db.session.commit()
    flash(f'Venue {venueName} was successfully deleted!')
  except Exception:
    db.session.rollback()
    flash(f'please try again. Venue {venueName} could not be deleted.')
  finally:
    db.session.close()
...
def create_show_submission():
  ...
  for i in range(len(DTList)):
    DTList[i] = int(DTList[i])  # type: ignore
  new_show.start_time = datetime(DTList[0],DTList[1],DTList[2]  # type: ignore
                                        ,DTList[3],DTList[4],DTList[5])  # type: ignore
  now = datetime.now()
